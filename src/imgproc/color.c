...
at_arrayu8_cvt_color(AtArrayU8 *array, AtColorType from, AtColorType to){
  ...
  if(from == to)
    ...
    size = array->h.elemsize*array->h.nelem;
    if(to==AT_RGBA || to == AT_BGRA){
      for(i = 0,k = 0;k < array->h.nelem; i+=4,k++){
    ...
    case AT_ARGB:
      ...
      break;
    case AT_GRAY:
      for(i = 0,j = 0; i < array->h.nelem; i+=4,j++){
    ...
    case AT_RGBA:
      ...
      break;
    case AT_BGRA:
      for(i = 0, j = 0;i < array->h.nelem; i+=3, j+=4){
    ...
    case AT_GRAY:
      for(i = 0,j = 0; i < array->h.nelem; i+=3,j++){
        newar->data[j  ] = array->data[i  ]*0.299 +
                           array->data[i+1]*0.587 +
                           array->data[i+2]*0.114;
      }
...
at_arrayu8_lut_random_multi(AtArrayU8* array, uint8_t nchannels){
  ...
  for(i = 0,k=0; i < arraylut->h.nelem; i+=nchannels,k++)
...
at_arrayu8_add_brightness(AtArrayU8* array, int16_t pos){
  ...
  for(i = 0; i < array->h.nelem; i+=totalchan){
